"use strict";
const _i = ['Phigros模拟器', [1, 4, 3], 1611795955, 1630855949];
...
class Click {
	...
	static activate(offsetX, offsetY) {
		...
		if (offsetX > canvas.width - lineScale * 1.5 && offsetY < lineScale * 1.5) {
			specialClick.click(1);
		}
		if (offsetX > canvas.width - lineScale * 1.5 && offsetY > canvas.height - lineScale * 1.5) {
...
canvas.addEventListener("mousedown", evt => {
	...
	out.innerText = idx + " " + clicks[idx] + " " + clicks[idx].isActive;
});
...
function init() {
	...
	out.innerText = "This is a test text...";
	loadResourse();
	//加载资源
	async function loadResourse() {
		out.innerText = "load resourse invoked...";
		try {
			let loadedNum = 0;
			await Promise.all((obj => {
				...
				out.innerText = "requesting src...";
				const xhr = new XMLHttpRequest();
				xhr.open("get", src, true);
				if (/\.(png|jpeg|jpg)$/i.test(src)) {
					out.innerText = "matched image";
					xhr.responseType = 'blob';
					xhr.send();
					out.innerText = "sended request";
					xhr.addEventListener("load", async () => {
						const resp = xhr.response
						res[name] = await createImageBitmap(resp);
						out.className = "accept";
					...
					xhr.addEventListener("load", async () => {
						const resp = xhr.response;
						res[name] = await actx.decodeAudioData(resp);
						out.className = "accept";
						out.innerText = `加载资源：${Math.floor(++loadedNum / arr.length * 100)}%`;
						resolve();
					});
